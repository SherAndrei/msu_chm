...
static void fill_k1(double* f, const double* y, double x, double h, unsigned n, double* k1)
{
	RightPartOfEquations(f, y, x);
	fill_k_impl(n, h, f, k1);
}

static void fill_k2(double* f, double* y_tmp, const double* y, double x, double h, unsigned n, const double* k1, double* k2)
{
	for (unsigned i = 0u; i < n; i++)
		y_tmp[i] = y[i] + k1[i] / 2.;
...
static void fill_k3(double* f, double* y_tmp, const double* y, double x, double h, unsigned n, const double* k2, double* k3)
{
	for (unsigned i = 0u; i < n; i++)
		y_tmp[i] = y[i] + 3. * k2[i] / 4.;
...
void Step(double* yn, const double* y, unsigned n, double x, double h)
{
	...
	fill_k1(f, y, x, h, n, k1);
	fill_k2(f, y_tmp, y, x, h, n, k1, k2);
	fill_k3(f, y_tmp, y, x, h, n, k2, k3);

	for (unsigned i = 0u; i < n; i++) {
		yn[i] = y[i] + (2. * k1[i] + 3. * k2[i] + 4. * k3[i]) / 9.;
	}
