...
static void fill_k1(double* f, const double* yn, double x, double h, unsigned n, double* k1)
{
	RightPartOfEquations(f, yn, x);
	fill_k_impl(n, h, f, k1);
}

static void fill_k2(double* f, double* y_tmp, const double* yn, double x, double h, unsigned n, const double* k1, double* k2)
{
	for (unsigned i = 0u; i < n; i++)
		y_tmp[i] = yn[i] + k1[i] / 2.;
...
static void fill_k3(double* f, double* y_tmp, const double* yn, double x, double h, unsigned n, const double* k2, double* k3)
{
	for (unsigned i = 0u; i < n; i++)
...
void Step(double* y_curr, const double* y_prev, unsigned n, double x, double h)
{
